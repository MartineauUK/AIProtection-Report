...
#                     ['date='[date_string[,...]]] ['time='[url_string[,...]] ['backup[=directory]'] ['nofilter'] ['email']  ['mode=or'] ['noscript']
#                     ['count'] ['sortby='column] ['report='{file_name}] ['nodisplay'] ['showsql']
#
#    AiProtection
...
#    AiProtection     nofilter report=AiProtection.csv nodisplay
#                     Will list ALL entries in the AiProtection Monitor database and will write results to file 'AiProtection.csv'
...
if [ -z "$CMDNOSCRIPT" ];then
    ...
    if [ -n "$CMDCOUNT" ];then
        [ $SHOWSQL -eq 1 ] && echo -e "sqlite3 $SQL_DATABASE SELECT datetime(timestamp, 'unixepoch', 'localtime') AS time,  count(*) FROM $SQL_TABLE $WHERE ORDER BY time;\n"   # v1.13
        ...
        if [ "$CMDREPORT" = "CreateCSV" ] && [ "$CMDNODISPLAY" = "NoDISPLAY" ];then     # v1.12
            [ $SHOWSQL -eq 1 ] && echo -e "sqlite3 -csv $SQL_DATABASE SELECT datetime(timestamp, 'unixepoch', 'localtime') AS time, mac, src, dst, id, dir, severity FROM $SQL_TABLE $WHERE ORDER BY $SORTBY;\n"    # v1.13
            sqlite3 -csv $SQL_DATABASE "SELECT datetime(timestamp, 'unixepoch', 'localtime') AS time, mac, src, dst, id, dir, severity FROM $SQL_TABLE $WHERE ORDER BY $SORTBY;" > $REPORT_CSV
            [ $SHOWSQL -eq 1 ] && echo -e "sqlite3 $SQL_DATABASE SELECT datetime(timestamp, 'unixepoch', 'localtime') AS time, mac, src, dst, id, dir, severity, count(*) FROM $SQL_TABLE $WHERE ORDER BY $SORTBY;\n"   # v1.13
            RESULT_CNT=$(sqlite3 $SQL_DATABASE "SELECT datetime(timestamp, 'unixepoch', 'localtime') AS time, mac, src, dst, id, dir, severity, count(*) FROM $SQL_TABLE $WHERE ORDER BY $SORTBY;"  | cut -d'|' -f8)
            nvram set tmp_WH_TOTAL=$RESULT_CNT
        else
            [ $SHOWSQL -eq 1 ] && echo -e "sqlite3 $SQL_DATABASE SELECT datetime(timestamp, 'unixepoch', 'localtime') AS time, mac, src, dst, id, dir, severity FROM $SQL_TABLE $WHERE ORDER BY $SORTBY;\n" # v1.13
            sqlite3 $SQL_DATABASE "SELECT datetime(timestamp, 'unixepoch', 'localtime') AS time, mac, src, dst, id, dir, severity FROM $SQL_TABLE $WHERE ORDER BY $SORTBY;" | while IFS= read -r LINE
                do
                    ...
                    if [ "$CMDNODISPLAY" != "NoDISPLAY" ];then                  # v1.12
                        printf '%b %-1s %b %-10s %b %-8s %b %-18s %b%-17s %b %-16s %b %-7s %2s %b %-45s %b%-44s\n' "$cBBLU" "$SEV" "$COLOUR_DATE" "$DATE"  "$COLOUR_TIME" "$TIME" "$COLOUR_MAC" "$MAC" "$cBBLU" "$HOSTNAME" "$COLOUR_IP" "$IP" "$cBBLU" "$ID" "$DIR" "$COLOUR_SRC" "$SRC" "$COLOUR_DST" "$DST"  # v1.12     
                    fi
    ...
    if [ -z "$CMDCOUNT" ];then                                                  # v1.10
        if [ "$CMDREPORT" = "CreateCSV" ];then                                  # v1.12
            ...
            [ $SHOWSQL -eq 1 ] && echo -e "sqlite3 $SQL_DATABASE SELECT datetime(timestamp, 'unixepoch', 'localtime') AS time, timestamp, mac, src, dst, id, dir, severity FROM $SQL_TABLE $WHERE;\n"   # v1.13
